CREATE TABLE accept (id BIGINT AUTO_INCREMENT, question_id BIGINT UNIQUE NOT NULL, answer_id BIGINT UNIQUE NOT NULL, created_at DATETIME NOT NULL, INDEX question_id_idx (question_id), INDEX answer_id_idx (answer_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE answer (id BIGINT AUTO_INCREMENT, question_id BIGINT NOT NULL, user_id BIGINT NOT NULL, body TEXT, body_html TEXT, created_at DATETIME NOT NULL, votes BIGINT DEFAULT 0, accepted TINYINT(1) DEFAULT '0', INDEX question_id_idx (question_id), INDEX user_id_idx (user_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE question (id BIGINT AUTO_INCREMENT, user_id BIGINT NOT NULL, title VARCHAR(255) NOT NULL, body TEXT, body_html TEXT, views BIGINT DEFAULT 0 NOT NULL, interested_users BIGINT DEFAULT 0, answer_count BIGINT DEFAULT 0, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, INDEX user_id_idx (user_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE question_tag (id BIGINT AUTO_INCREMENT, question_id BIGINT NOT NULL, tag_id BIGINT NOT NULL, INDEX question_id_idx (question_id), INDEX tag_id_idx (tag_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE static_content (id BIGINT AUTO_INCREMENT, title VARCHAR(255) NOT NULL, path VARCHAR(50) NOT NULL UNIQUE, body TEXT, PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE tag (id BIGINT AUTO_INCREMENT, name VARCHAR(100) NOT NULL UNIQUE, PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE question_tag ADD CONSTRAINT question_tag_tag_id_tag_id FOREIGN KEY (tag_id) REFERENCES tag(id) ON DELETE CASCADE;
ALTER TABLE question_tag ADD CONSTRAINT question_tag_question_id_question_id FOREIGN KEY (question_id) REFERENCES question(id) ON DELETE CASCADE;
